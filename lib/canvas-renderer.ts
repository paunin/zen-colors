...
import { parseColor, createOffscreenCanvas, getContext2D, clamp, degToRad, noise2D } from './utils';
...
function createPolyTexture(
  r: number, g: number, b: number,
  radius: number, corners: number,
  rotation: number, time: number, seed: number,
  morphSpeed: number, morphRange: number
): TextureResult {
  const halfAmp = (morphRange - 1) / 2;
  const maxScale = 1 + halfAmp;
  const dim = Math.ceil(radius * 2 * maxScale * 1.15);
  const canvas = createOffscreenCanvas(dim, dim);
  const ctx = getContext2D(canvas);
  if (ctx) {
    const cx = dim / 2;
    ...
    const n = Math.max(3, Math.min(12, corners));
    ...
    const gradRadius = radius * maxScale;
    const gradient = ctx.createRadialGradient(0, 0, 0, 0, 0, gradRadius);
    gradient.addColorStop(0, `rgba(${r}, ${g}, ${b}, 1)`);
    gradient.addColorStop(0.4, `rgba(${r}, ${g}, ${b}, 0.8)`);
    gradient.addColorStop(0.7, `rgba(${r}, ${g}, ${b}, 0.3)`);
    gradient.addColorStop(1, `rgba(${r}, ${g}, ${b}, 0)`);
    ctx.fillStyle = gradient;
    const fill = gradRadius + 1;
    ctx.fillRect(-fill, -fill, fill * 2, fill * 2);
  }
  return { canvas, width: dim, height: dim };
}

function createBlobTexture(config: BlobConfig, radius: number, time?: number, seed?: number): TextureResult {
  ...
  switch (shape) {
    ...
    case 'poly':
      return createPolyTexture(
        r, g, b, radius, config.corners ?? 6, rotation,
        time ?? 0, seed ?? 0, config.morphSpeed ?? 0.3, config.morphRange ?? 2.5
      );
    case 'circle':
    default:
...
function needsTextureRebuild(blob: BlobState, drawRadius: number): boolean {
  ...
  if ((blob.config.shape ?? 'circle') === 'poly') return true;
...
export function renderFrame(params: RenderParams): void {
  ...
  for (const blob of blobs) {
    ...
    if (needsTextureRebuild(blob, drawRadius)) {
      const result = createBlobTexture(blob.config, drawRadius, adjustedTime, blob.noiseOffsetX);
